...
} from "./services/pendingOrdersSync";
import {
	CartItem,
	Client,
	DraftDetails,
...
interface AppContextType {
	...
	clearCart: () => void;
	saveDraft: (details: DraftDetails) => void;
	...
	setDeployEvent: (event: DeployEvent | null) => void;
	setAppVersionInfo: (info: AppVersionInfo | null) => void;
	...
	setDeployNotification: (event: DeployEvent | null) => void;
...
export function AppProvider({ children }: { children: ReactNode }) {
	...
	const [currentSeller] = useState<Seller | null>(null);
	...
	>("online");
	const [shownNotificationIds, setShownNotificationIds] = useState<Set<number>>(
		new Set(),
	);
	const [supabaseUser, setSupabaseUser] = useState<{ id?: string; email?: string } | null>(null);
	const [setPendingOrdersCount] = useState<number>(0);

	const { fetchNotifications, sendNotification: sendNotificationBase } =
		useNotifications(
			globalPin,
			currentSeller,
			setNotifications,
	...
	const { forceRefresh: forceRefreshBase, clearAllCaches } =
		useCacheManager(refreshData);
	...
	const _syncAll = useCallback(async () => {
	...
	const setDeployNotification = (event: DeployEvent | null) => {
	...
	const updateTheme = useCallback(
		(newTheme: "light" | "dark", isManual = true) => {
			setTheme(newTheme);
	...
	const syncPendingOrders = useCallback(async (sellerId: string) => {
			...
			try {
				...
				if (result.total > 0) {
					console.log(
						`✅ Synced ${result.success} orders, ${result.failed} failed`,
					);
		...
		[isOnline, refreshData, setPendingOrdersCount],
	);
	...
	useEffect(() => {
		...
		const loadCachedAndRefresh = async () => {
			...
			try {
				...
				const cachedSharedCarts = await get<SharedCart[]>(
					"tecnogafas_shared_carts",
				);
				...
				if (cachedSharedCarts) {
					setSharedCarts(cachedSharedCarts);
				}
				if (cachedOrdersData) {
			...
			const savedTheme = localStorage.getItem("tecnogafas_theme") as
				| "light"
				| "dark"
				| null;
			const isManual =
				localStorage.getItem("tecnogafas_theme_manual") === "true";
	...
	const globalChannelRef = React.useRef<{ unsubscribe: () => void } | null>(null);
		...
		const setupGlobalNotifications = async () => {
			try {
				...
				const { data: conv, error: convError } =
					await apiService.getSupabaseNotificationChannel();
				if (convError || !conv) {
				...
				const { data: member, error: memberError } =
					await apiService.getSupabaseMemberStatus(conv.id, supabaseUser.id);
									...
									notifications: [
										{
											...
											body:
												msg.content || "Tienes un nuevo mensaje del sistema",
											id: Date.now(),
	...
	const saveDraft = async (details: DraftDetails) => {
	...
	const shareCart = async () => {
		if (!selectedClient || cart.length === 0) {
			return {
				success: false,
				code: "",
				message: "Carrito vacío o sin cliente",
				link: "",
			};
		}
		...
		try {
			...
			return {
				success: true,
				code,
				message: "Carrito compartido exitosamente",
				link,
			};
		} catch (error) {
			...
			return {
				success: false,
				code: "",
				message: "Error al generar enlace compartido",
				link: "",
			};
		}
	};
	...
	const loadSharedCart = async (
		...
		try {
			...
			const sharedCartData = data as { id: string; code: string; client?: Client; items: CartItem[]; total: number; created_at: string; expires_at: string; is_active: boolean } | null;
			...
			if (new Date(sharedCartData.expires_at) < new Date()) {
				return {
					success: false,
					cart: null,
					message: "Este carrito ha expirado",
				};
			}
			...
			// Update local cache
			const updatedSharedCarts = [
				mappedCart,
				...sharedCarts.filter((c) => c.id !== mappedCart.id),
			];
			...
			return {
				success: false,
				cart: null,
				message: "Error al cargar carrito compartido",
			};
		}
	};
		...
		<AppContext.Provider
			value={{
				...
				syncPendingOrders: async () => {
					if (globalPin) await syncPendingOrders(globalPin);
				},
			}}
		>
			{children}
