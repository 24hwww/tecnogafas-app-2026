...
export async function getPendingOrders(
	...
	try {
		...
		if (error) {
			console.error(
				"[PendingOrdersSync] Error fetching pending orders:",
				error,
			);
			return [];
		}
		...
		console.error(
			"[PendingOrdersSync] Exception fetching pending orders:",
			err,
		);
		return [];
	}
...
export async function syncPendingOrder(
	...
	try {
		...
		if (result.success && result.orderId) {
			...
			console.log(
				`[PendingOrdersSync] Order ${order.id} synced successfully as ${result.orderId}`,
			);
		...
		console.error(
			`[PendingOrdersSync] Exception syncing order ${order.id}:`,
			msg,
		);
		return { success: false, error: msg };
	}
...
export async function syncAllPendingOrders(
	...
	console.log(
		`[PendingOrdersSync] Starting sync of ${orders.length} pending orders`,
	);
...
export async function updatePendingOrder(
	...
	try {
		...
		if (updates.client)
			updateData.client_data = updates.client as unknown as Record<
				string,
				unknown
			>;
		if (updates.items)
			updateData.items = updates.items as unknown as Record<string, unknown>[];
		if (updates.details)
			updateData.details = updates.details as unknown as Record<
				string,
				unknown
			>;
