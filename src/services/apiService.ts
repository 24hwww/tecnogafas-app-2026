...
const customFetch = async (
	...
	try {
		...
		if (error.name === "AbortError") {
			...
			window.dispatchEvent(
				new CustomEvent("api-error", {
					detail: { message: "Tiempo de espera agotado" },
				}),
			);
		} else {
			...
			window.dispatchEvent(
				new CustomEvent("api-error", {
					detail: {
						message: "No se pudo conectar con la API (caída o sin red)",
					},
				}),
			);
		}
...
export const apiService = {
				...
				.map((v: string) => {
					...
					if (titleMatch) {
						...
						const titlePart = parts.find(
							(p) => p.startsWith("Título:") || p.startsWith("Titulo:"),
						);
	...
	async getOrders(
		...
		const orders = (json.data || []).map((o) => ({
			...
			status: (o.post_status === "unattended"
				? "Pendiente"
				: "Completado") as Order["status"],
			...
			rawData: {
				...
				customer_note:
					o.observaciones ||
					o.customer_note ||
					(o as ApiOrder & { notes?: string }).notes ||
					"",
			},
		}));
	...
	async verifyProducts(
		...
		try {
			...
			return {
				success: false,
				message: "Error de conexión durante verificación",
			};
		}
	},
	...
	async getSellers(): Promise<Seller[]> {
		...
		if (!res.ok) throw new Error(`API error: ${res.status}`);
		const json = (await res.json()) as {
			data?: Array<{ ID?: number; display_name?: string }>;
		};
	...
	async getEvents(type?: string, sellerId?: string): Promise<ApiEvent[]> {
		...
		if (
			!type &&
			cachedEvents &&
			Date.now() - cachedEventsTimestamp < CACHE_EVENTS_TTL
		) {
	...
	async createOrder(
		...
		const bodyObj = {
			...
			iva: details.iva ? parseInt(String(details.iva), 10) : 0,
			...
			products: items
				.map((i) => {
					...
					return {
						product_id: parsedId,
						variation_id: i.vid ? parseInt(i.vid, 10) : undefined,
						quantity: i.quantity,
						price: i.price,
					};
				})
		...
		try {
			...
			if (!msg)
				msg = res.ok ? "Pedido creado con éxito" : "Error al crear el pedido";
			...
			return {
				success: false,
				message: `${msg} (El pedido se guardó para reintentar)`,
			};
		}
	},
	...
	async loginSeller(pin: string): Promise<Seller | null> {
		const res = await customFetch(
			`${BASE_URL}/login?data=${encodeURIComponent(pin)}`,
			{
				method: "POST",
			},
		);
		...
		try {
			...
			if (!res.ok)
				return {
					success: false,
					message:
						data.message || `Error al enviar email (Status: ${res.status})`,
				};
	...
	subscribeToEvents(onMessage: (data: ApiEvent) => void) {
		...
		eventSource.onmessage = (event) => {
			try {
				...
				if (data.type === "order") {
					console.log(
						"🔄 Evento de orden recibido, invalidando caché y refrescando pedidos...",
					);
	...
	async getSupabaseNotificationChannel(): Promise<{
		data: { id: string } | null;
		error: any;
	}> {
	...
	async getSupabaseMemberStatus(
		...
		userId: string,
	): Promise<{ data: { unread_count: number } | null; error: any }> {
		const { supabase } = await import("../modules/chat/lib/supabase");
		return supabase
	...
	async syncSupabaseAuth(pin: string) {
		...
		if (
			...
			if (signUpError) {
				console.error(
					"❌ Error en auto-registro Supabase:",
					signUpError.message,
				);
