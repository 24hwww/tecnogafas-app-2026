...
import type { DraftOrder, Order } from "../types";
...
export default function Orders() {
	...
	const _getSellerName = (sellerId: string) => {
	...
	const [actionType, setActionType] = useState<
		"pdf" | "email" | "status" | "regenerar" | null
	>(null);
	...
	const targetId =
		location.state?.highlightOrderId || location.state?.newOrderId;
	...
	const loadOrders = async (sellerId?: string, customerId?: string) => {
		await fetchOrders(
			1,
			100,
			sellerId ? parseInt(sellerId, 10) : undefined,
	...
	useEffect(() => {
		if (orders.length > 0) {
			console.log(
				"DEBUG: Primer pedido post_title:",
				orders[0].rawData?.post_title,
			);
		}
	}, [orders]);

	const handleActionClick = async (
		type: "pdf" | "email" | "status" | "regenerar",
	) => {
		setActionType(type);
		if (globalPin) {
	...
	const executeAction = async (
		...
		try {
			...
			if (!sellerInfo) {
				setActionFeedback({
					message: "PIN incorrecto o error de red",
					type: "error",
				});
			...
			if (currentAction === "pdf") {
				const success = await apiService.downloadOrderPdf(
					selectedOrder.id,
					sellerInfo.id,
				);
				if (success) {
				...
				const res = await apiService.sendOrderEmail(
					selectedOrder.id,
					sellerInfo.id,
				);
				if (res.success) {
				...
				const newStatus =
					selectedOrder.status === "Pendiente" ? "attended" : "unattended";
				...
				if (res.success) {
					setPinModalOpen(false);
					alert(
						`Estado actualizado a ${newStatus === "attended" ? "Completado" : "Pendiente"}`,
					);
				...
				try {
					...
					if (res.success) {
						...
						setActionFeedback({
							message: res.message || "Error al regenerar el pedido",
							type: "error",
						});
					}
				} catch (e: any) {
					setActionFeedback({
						message: e.message || "Error al regenerar",
						type: "error",
					});
				} finally {
		...
		setTimeout(async () => {
			if (receiptRef.current) {
				try {
					...
					if (blob) {
						const file = new File([blob], `Pedido_${draft.id.slice(-6)}.png`, {
							type: "image/png",
						});
			...
			<div className="space-y-8 min-h-[50vh]">
				...
				{hasDrafts && (
					<div className="space-y-4">
						<div className="flex items-center justify-between">
							<h2 id="orders-drafts-title" className="text-2xl font-bold">
								Borradores
							</h2>
						...
						<div className="space-y-3">
							{drafts
								.filter((d) => d.status === "no enviado")
								.map((draft) => (
									<div
										key={draft.id}
										className="card-premium p-0 overflow-hidden border-dashed border-2"
									>
										<div className="p-4 border-b flex justify-between items-center bg-secondary/50">
											<div>
												<p className="font-bold text-sm">
													Borrador #{draft.id.slice(-6)}
												</p>
												<div className="flex items-center gap-2">
													<p className="text-[0.625rem] text-muted-foreground font-mono font-medium flex gap-1 items-center uppercase">
														{new Date(draft.date).toLocaleDateString("es-AR", {
															timeZone: "America/Argentina/Buenos_Aires",
															day: "2-digit",
															month: "2-digit",
														})}{" "}
														{formatTimeBA(draft.date)} HS –{" "}
														{getRelativeTime(draft.date)}
													</p>
												</div>
											</div>
										...
										<div className="p-4 flex justify-between items-center bg-background gap-2">
											<div className="flex-1 min-w-0">
												<p className="text-[0.625rem] text-muted-foreground uppercase tracking-wider font-bold mb-1">
													Cliente
												</p>
											...
											<div className="flex gap-2 shrink-0">
												...
												>
													REVISAR <Send size={12} className="ml-2" />
												</Button>
											</div>
										</div>
									</div>
				...
				<div className="space-y-4">
						...
						<button
							...
							onClick={() =>
								fetchOrders(
									1,
									100,
									selectedSeller ? parseInt(selectedSeller, 10) : undefined,
									selectedCustomer ? parseInt(selectedCustomer, 10) : undefined,
								)
							}
							disabled={isLoading}
					...
					<div className="space-y-3">
						{isLoading ? (
							Array(4)
								.fill(0)
								.map((_, i) => <OrderSkeleton key={`skeleton-${i}`} />)
						) : filteredOrders.length === 0 ? (
							<p className="text-center p-4 text-on-surface-variant text-sm">
								No se encontraron pedidos
							</p>
						) : (
							filteredOrders.map((order) => {
								const _isHighlight =
									targetId?.toString() === order.id.toString();
								return (
									<Card
										key={order.id}
									...
									>
											...
											<div className="flex-1 min-w-0 pr-2">
												<div className="flex items-center gap-2">
													<p className="font-bold text-sm truncate">
														{order.rawData?.post_title || order.clientName}
													</p>
												</div>
												<div className="flex items-center gap-2 mt-0.5">
													<span className="text-[0.625rem] text-muted-foreground font-mono font-medium uppercase">
										...
										<Button
											...
											className="w-full h-12 rounded-none font-bold tracking-widest"
										>
											VER DETALLES
										</Button>
									</Card>
					...
					<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
						<div className="m3-card !bg-surface w-full max-w-lg max-h-[90vh] overflow-y-auto space-y-4">
							<div className="flex justify-between items-center border-b border-outline/10 pb-4">
								...
								>
									{selectedOrder.rawData?.post_title ||
							...
							<div className="space-y-4 text-sm bg-surface-variant/20 p-4 rounded-lg">
								<div className="grid grid-cols-2 gap-4">
									<div>
										<p className="text-[0.625rem] uppercase font-bold text-outline">
											Estado
										</p>
									...
									<div>
										...
										<p className="font-medium">
											{formatDateTimeBA(selectedOrder.createdAt)}
										</p>
									</div>
									<div>
										<p className="text-[0.625rem] uppercase font-bold text-outline">
									...
									<div>
										...
										<p className="font-medium">
											{selectedOrder.rawData?.transport || "Sin definir"}
										</p>
									</div>
									<div>
										<p className="text-[0.625rem] uppercase font-bold text-outline">
								...
								<div className="pt-3 border-t border-outline/10">
									<div className="flex justify-between text-xs mb-1">
										...
										<span className="font-medium">
											{selectedOrder.rawData?.discount || 0}%
										</span>
									</div>
									<div className="flex justify-between text-xs mb-1">
										<span className="text-outline">Recargo</span>
										<span className="font-medium">
											{selectedOrder.rawData?.recargo || 0}%
										</span>
									</div>
									<div className="flex justify-between text-xs">
										<span className="text-outline">IVA</span>
										<span className="font-medium">
											{selectedOrder.rawData?.iva || 21}%
										</span>
									</div>
								</div>
							</div>
								...
								<div className="max-h-[200px] overflow-y-auto pr-1 space-y-1">
									{selectedOrder.items.map((item, i) => (
										<div
											...
											className="flex justify-between text-xs py-2 border-b border-outline/5 last:border-0"
										>
				...
				{pinModalOpen && (
					<div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
						<div
							...
							onClick={() => !isActionLoading && setPinModalOpen(false)}
						/>
						<div className="relative bg-surface w-full max-w-xs p-6 shadow-2xl text-center space-y-4 rounded-xl">
							...
							<p className="text-sm text-on-surface-variant">
								Ingrese su PIN de vendedor para{" "}
								{actionType === "pdf" ? "descargar" : "enviar"} el pedido
							</p>
							<input
								type="password"
								value={pin}
							...
							{actionFeedback && (
								<p
									className={`text-xs font-bold ${actionFeedback.type === "error" ? "text-red-500" : "text-primary"}`}
								>
				...
				<div className="absolute top-[-9999px] left-[-9999px]">
					{sharingDraftId && (
						<div
							ref={receiptRef}
							className="bg-white text-black p-6 w-[400px] shadow-lg rounded-xl flex flex-col font-sans"
						...
						>
							{(() => {
								...
								const discountAmount =
									(subtotal * (Number(d.details?.discount) || 0)) / 100;
								const recargoAmount =
									(subtotal * (Number(d.details?.recargo) || 0)) / 100;
								const baseForIva = subtotal - discountAmount + recargoAmount;
								const ivaAmount =
									(baseForIva * (Number(d.details?.iva) || 0)) / 100;
									...
									<>
										<div className="text-center border-b border-gray-200 pb-4 mb-4">
											...
											<p className="text-sm text-gray-500">
												#{d.id.slice(-6)} - {formatDateTimeBA(d.date)}
											</p>
										</div>
										<div className="mb-4">
											<p className="text-sm">
												<strong className="uppercase">Cliente:</strong>{" "}
												{d.client.name}
											</p>
											<p className="text-sm">
										...
										<div className="mb-4 border-b border-gray-200 pb-4">
											<h2 className="font-bold text-sm uppercase tracking-wider mb-2">
												Productos
											</h2>
