    ...
    <>
      ...
      <div className="flex items-center justify-between px-5 py-5">
        ...
        <button
          ...
          className="btn btn-ghost btn-square btn-xs lg:hidden"
        >
          <X size={16} />
        </button>
      </div>
          ...
          return (
            ...
            >
              ...
              {item.label === 'Chat' && unread > 0 && pathname !== '/chat' && (
                <span className="badge badge-sm badge-error animate-pulse">
                  {unread > 99 ? '99+' : unread}
                </span>
              )}
            </NavLink>
      ...
      <AnimatePresence>
        {isLoading && (
          <motion.div
            ...
            className="absolute inset-0 bg-base-100/90 backdrop-blur-xl z-[100] flex flex-col items-center justify-center"
          >
            <RefreshCw className="w-8 h-8 text-primary animate-spin mb-3" />
            <p className="text-xs font-semibold text-primary animate-pulse tracking-widest uppercase">
              Sincronizando...
            </p>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="flex items-center gap-2 px-4 h-14 bg-[var(--color-surface-900)]/80 backdrop-blur-lg sticky top-0 z-30 border-b border-[var(--color-border)] shrink-0">
          ...
          <div className="hidden lg:flex flex-1 max-w-md relative">
            ...
            <input
              ...
              className="input input-sm input-bordered w-full pl-9 bg-[var(--color-surface-800)] border-[var(--color-border)] text-sm placeholder:text-[var(--color-text-muted)]"
            />
          </div>
          ...
          <button
            type="button"
            onClick={() => navigate('/')}
            className="flex items-center gap-2 lg:hidden flex-1 min-w-0"
          >
            <span className="text-sm font-bold tracking-tight text-base-content truncate">
              Tecnogafas
            </span>
          </button>
          ...
          <div className="flex items-center gap-1">
            ...
            {onlineUsersCount !== null && (
              <div className="badge badge-sm bg-primary/10 text-primary border-primary/20 gap-1">
                <Users2 size={11} />
                <span className="text-[10px] font-semibold">{onlineUsersCount}</span>
              </div>
            )}
            <button
              type="button"
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="badge badge-primary badge-xs absolute -top-0.5 -right-0.5 border-2 border-base-100 text-[9px]">
                  {cartCount}
                </span>
              )}
            </button>
        ...
        <AnimatePresence>
          {apiError && (
            <motion.div
              ...
              className="alert alert-error rounded-none py-2 text-sm"
            >
              <AlertCircle size={14} />
              <span className="flex-1 truncate text-xs">{apiError}</span>
              <button
                type="button"
        ...
        <main className="flex-1 overflow-y-auto">
          <OptimizedPageTransition pathname={location.pathname}>
            <div className="p-4 lg:p-6 xl:p-8 min-h-full max-w-7xl mx-auto w-full">{children}</div>
