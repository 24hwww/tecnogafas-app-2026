...
export default function ChatTest() {
	...
	const checkAuth = async () => {
		...
		setCurrentUser(user);
		// biome-ignore lint: Supabase user logging
		// biome-ignore lint: Supabase auth UID logging
	...
	const testConnection = async () => {
		...
		if (!user) {
			...
			setStatus("error");
			setError(
				"No hay sesión de Supabase y no se encontró un PIN configurado.",
			);
			return;
		}
	...
	const sendTestMessage = async () => {
		...
		if (authError || !user) {
			setError(
				`❌ No autenticado en Supabase. Auth UID: ${user?.id || "null"}`,
			);
	...
	const sendSystemNotification = async () => {
		try {
			...
			if (!conv) {
				setError(
					"Canal #notificaciones no encontrado. Ejecuta el SQL primero.",
				);
				return;
			}
